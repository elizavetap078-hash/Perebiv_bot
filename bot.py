...
try:
    TIMER_UPDATE_SECONDS = max(
        1,
        int(os.getenv("TIMER_UPDATE_SECONDS", "20"))
    )
except ValueError:
    TIMER_UPDATE_SECONDS = 20
...
if not ADMIN_IDS:
    print(
        "WARNING: ADMIN_IDS не задан. "
        "Управление ботом будет недоступно.",
        flush=True,
    )
...
async def require_admin(
    ...
    if not is_admin_user(user_id):
        await callback.answer(
            "❌ У тебя нет прав управления ботом.",
            show_alert=True
        )
...
async def ensure_bot_admin(chat_id: int) -> bool:
    """
    Проверяет, является ли сам бот администратором группы.
    """

    return await bot_is_admin(chat_id)
...
async def choose_new_chat(
    ...
    if not await ensure_bot_admin(chat_id):
        await callback.message.answer(
            "❌ Бот больше не является "
...
async def game_description(
    ...
    if "duration" not in data or "prize" not in data:
        await state.clear()

        await message.answer(
            "❗ Данные игры потеряны. "
            "Начни создание игры заново."
        )
        return
...
async def publish_game(
    ...
    if not await ensure_bot_admin(chat_id):
        await callback.message.answer(
            "❌ Бот больше не является "
            "администратором выбранной группы."
        )
        await state.clear()
        return
    ...
    try:
        ...
        print(
            f"Ошибка публикации игры {chat_id}: "
            f"{repr(e)}",
            flush=True,
        )
...
async def group_handler(
    ...
    # Любое сообщение в группе автоматически
    ...
    # Администраторы группы никогда не перебивают.
...
async def mark_event_finished(
    chat_id,
    winner_text=None
):
    """
    Обновляет старое сообщение ивента,
    чтобы оно больше не выглядело активным.
    """
    ...
    if winner_text:
        ...
        text = (
            ...
            f"🎁 Приз: {game['prize']}\n"
            f"⏱ Длительность: "
            f"{format_time(game['duration'])}\n\n"
            "Новая игра ещё не запущена."
        )
    ...
    try:
        ...
        print(
            "Finished event message edit error:",
            repr(e),
            flush=True,
        )
...
if __name__ == "__main__":
    try:
        ...
        print(
            "Perebiv_bot stopped by keyboard.",
            flush=True
        )
